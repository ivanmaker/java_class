...
public class XMLUtilsTest {
    ...
    public void testUnmarshall() throws Exception {
        Stock stock = XMLUtils.unmarshall(xmlStocks, Stock.class);
        validateStock(stock);
    }

    @Test
    public void testUnmarshallWithSchemaValidation()throws Exception {
        Stock stock = XMLUtils.unmarshall(xmlStocks, Stock.class, "/xml/stock_info.xsd");
        validateStock(stock);
    }
    ...
    public void testMarshall() throws Exception {
        Stock stock = XMLUtils.unmarshall(xmlStocks, Stock.class, "/xml/stock_info.xsd");
        String xml = XMLUtils.marshall(stock);
        // input xml should be the same as output xml
        assertEquals("XML out is correct", xml.trim() ,xmlStocks.trim());
    }

    private void validateStock(Stock stock) {
        assertTrue("Symbol name is correct", stock.getSymbol().getContent().equals(""));
        assertTrue("Price name is correct", stock.getPrice().getContent().equals(""));
        assertTrue("Time is not null", stock.getTime().getContent().equals(""));
    }
